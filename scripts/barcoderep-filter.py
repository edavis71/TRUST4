...
with open(barcodeReport) as fp:
	for line in fp:
		...
		if (len(chain1Cols) > 1):
			if float(chain1Cols[6]) >= highAbund:
				cdr3 = chain1Cols[4]
		...
		if (len(chain2Cols) > 1):
			if float(chain2Cols[6]) >= highAbund:
				cdr3 = chain2Cols[4]
		...
		barcodeInfo[cols[0]] = {"chain1": chain1Cols[:], "chain2": chain2Cols[:]}
if (args.annot is not None):
	with open(args.annot) as fp:
		for line in fp:
			header = line.rstrip()
